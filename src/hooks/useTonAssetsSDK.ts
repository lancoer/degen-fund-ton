...
import { uploadMetadataToIPFS } from '@/lib/utils/uploadMetadataToIPFS';
...
/**
 * Use it to get ton assets sdk. If wallet is not connected hook will return null.
 */
const useTonAssetsSDK = (): AssetsSDK | null => {
  const [tonConnectUI] = useTonConnectUI();
  const [assetsSDK, setAssetsSDK] = useState<AssetsSDK | null>(null);
        ...
        const sdk = AssetsSDK.create({
          ...
          storage: {
            uploadFile: async (contents) => {
              const IpfsHash = await uploadMetadataToIPFS(contents.toString(), {
...
export default useTonAssetsSDK;
